...
extern "C"
{
    ...
    struct openvpn_plugin_string_list **return_list)
  {
    ...
    if ( verb_string ) {
      ...
      log.init_verbosity(context->getVerbosity());
    }
    ...
    log.debug() << "Start AUTH-RADIUS-PLUGIN\n";
    ...
    log.debug() << "Found  " <<  string_array_len(argv) << " params.\n";
    ...
    if ( string_array_len ( argv ) > base_parms )
    {
      ...
      if ( strncmp ( name_value_list.data[0].name,"-c",2 ) ==0 )
      {
        ...
        log() << "Configfile name: " << name_value_list.data[0].value << ".\n";
        ...
        {
          log() << "Bad config file or error in config.\n";
          goto error;
        }
      }
      else {
        log() << "OABad argument for plugin.\n";
        goto error;
      }
    }
    else
    {
      ...
      //parse the radiusplugin config file
      log() << "Configfile name:/etc/openvpn/radiusplugin.cnf.\n";
    ...
    if ( socketpair ( PF_UNIX, SOCK_DGRAM, 0, fd_auth ) == -1 ) {
      log() << " socketpair call failed for authentication process\n";
      goto error;
    }
    //Accounting process:
    if ( socketpair ( PF_UNIX, SOCK_DGRAM, 0, fd_acct ) == -1 ) {
      log()<< " socketpair call failed for accounting process\n";
      goto error;
    }
    ...
    if ( pid )
    {
      ...
      if ( fcntl ( fd_auth[0], F_SETFD, FD_CLOEXEC ) < 0 ) {
        log() << " Set FD_CLOEXEC flag on socket file descriptor failed\n";
      }

      log.debug() << "Start BACKGROUND Process for authentication with PID "
                  << context->getAuthPid() << ".\n";
      ...
      {
        log() << "Got not INIT SUCCEEDED while initing AUTH subprocess\n";
    ...
    { //Background Process
      ...
      //close the socket and free context
      close ( fd_auth[1] );
      delete context;
    ...
    { // Foreground Process (Parent)
      ...
      log.debug() << "Start BACKGROUND Process for accounting with PID " << context->getAcctPid() << ".\n";
      ...
      if ( fcntl ( fd_acct[0], F_SETFD, FD_CLOEXEC ) < 0 ) {
        log.debug() << "Set FD_CLOEXEC flag on socket file descriptor failed\n";
      }
      ...
      if ( status != RESPONSE_INIT_SUCCEEDED )
      {
        log() << "Got not INIT SUCCEEDED while initing ACCT subprocess\n";
        //set the socket to -1 if the initialization failed
        context->acctsocketbackgr.setSocket ( -1 );
      }
      log() << "Start AUTH-RADIUS-PLUGIN\n";
    }
    else
    { //Background Process
      ...
      log() << "Start BACKGROUND Process for accounting\n";
...
const char* StdLogger::date_fmt_ = "%a %b %d %T %Y";
...
void StdLogger::init_verbosity(int cur_verbosity)
{
  verb_ = cur_verbosity;
  debug_allowed_ = (logVerbThreshold <= cur_verbosity);
}
